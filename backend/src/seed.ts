...
async function run() {
  try {
    ...
    const insert = async (num: string, category: string, got: boolean) => {
      await pool.query(
        'INSERT INTO stickers (number, category, got) VALUES ($1, $2, $3) ON CONFLICT (number, category) DO NOTHING',
        [num, category, got]
      );
    };
    ...
    for (const n of regularGotTrue) await insert(n, 'REGULAR', true);
    for (const n of regularGotFalse) await insert(n, 'REGULAR', false);
    for (const n of especialesGotTrue) await insert(n, 'ESPECIAL', true);
    for (const n of especialesGotFalse) await insert(n, 'ESPECIAL', false);
    for (const n of troqueladosGotTrue) await insert(n, 'TROQUELADO', true);
    for (const n of troqueladosGotFalse) await insert(n, 'TROQUELADO', false);

    const cnt = await pool.query('SELECT COUNT(*)::int AS count FROM stickers');
